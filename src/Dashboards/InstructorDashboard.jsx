...
const InstructorDashboard = () => {
    const [view, setView] = useState("allClass")
    const { user } = useAuth()
    ...
    const onSubmit = (data, { setSubmitting }) => {
        ...
        .then(res=>res.json())
        .then(imageResponse=>{
            const newClass =  { available_seats: data.availableSeats, class_instructor_name:data.instructorName, class_title: data.className, image_link: imageResponse.data.display_url, price:data.price}
    ...
    return (
        <div className='pt-20'>
            <div className='text-center my-4'>
                <button className={` p-3 w-40 transition-colors rounded-tl-lg rounded-bl-lg duration-300 border-r ${view === "allClass" ? 'bg-[#fad932] font-semibold ' : 'bg-zinc-400'} `} onClick={() => handleAllClasses()}>All Classes</button>
